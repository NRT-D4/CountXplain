...
#             param.requires_grad = False
    ...
    def orthonormality_loss(self, similarity_threshold=0.):
        '''
    ...
    def training_step(self, batch, batch_idx):
        ...
        if torch.isnan(loss):
            print(f"Loss is NaN")
            print(f"fg_loss: {fg_loss}")
            print(f"diversity_loss: {diversity_loss}")
            print(f"bg_loss: {bg_loss}")
            print(f"bg_loss: {cell_loss}")
    ...
    def on_validation_epoch_end(self):
        if "DCC" in self.hparams["dataset"]:
            push_epoch = 100
        elif "IDCIA_v1" in self.hparams["dataset"]:
            push_epoch = 100
