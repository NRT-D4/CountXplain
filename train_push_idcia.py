...
def train(args):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=f"reb_outputs/{dataset}_64_{num_prototypes}/",
        filename=f"{dataset}_64_proto_{num_prototypes}_div_{diversity_coef}_proto_{proto_to_feature_coef}"+"_{val_counting_loss:.2f}_mae",
        save_top_k=1,
        mode="min",
    )

    wandb_logger = WandbLogger(
        project=f"MIDL - {dataset} - FINAL", name=f"push_{dataset}_64_{num_prototypes}_{lr}"
    )
...
def parse_args():
    ...
    parser.add_argument("--diversity_coef", type=float, default=0)
...
def main():
    ...
    # # run training with proto_to_feature_coef = 0
    # args.proto_to_feature_coef = 0
    # args.dataset = "DCC"
    # args.num_prototypes = 6

    # train(args)

    # # run training with diversity_coef = 0
    # args.diversity_coef = 0
    # args.dataset = "DCC"
    # args.num_prototypes = 6
    # args.proto_to_feature_coef = 1

    # train(args)

    # # Experiments on the IDCIA_v1 dataset
    # args.dataset = "IDCIA_v1"
    # args.num_prototypes = 8

    # # experiment with proto_to_feature_coef = 0
    # args.proto_to_feature_coef = 0
    # args.diversity_coef = 100

    # train(args)

    # # experiment with diversity_coef = 0
    # args.diversity_coef = 0
    # args.proto_to_feature_coef = 1
